...
#if 1 // make rounded corner
    const int w = LV_HOR_RES;
    const int h = LV_VER_RES;
    const int radius = 90;
    // draw a rectangle with border radius 50
    int pt_x = radius;
    int pt_y = radius;
    for (int y = 0; y < pt_y; y++)
    {
        for (int x = 0; x < pt_x; x++)
        {
            if (sqrt((x - pt_x) * (x - pt_x) + (y - pt_y) * (y - pt_y)) > radius)
            {
                // m->tft_fb[y * w + x] = 0xff000000 & m->tft_fb[y * w + x] + 0x33000000;
		//((uint32_t*)ximage->data)[y * w + x] &= 0x00ffffff;
                //((uint32_t*)ximage->data)[y * w + x] |= 0x33000000;
                ((uint32_t*)ximage->data)[y * w + x] |= 0xeeeeeeee;
            }
        }
    }
    pt_x = w - radius;
